...
library(Hmisc) # package that has helpful visualization tools for our data
  ...
  # Package options
options(gargle_oauth_email="c.h.w.chang@gmail.com") # default to "out-of-band" authentication for googlesheets4. TL;DR for this is that port 1410 which googlesheets4 uses to perform the authentication with Google's API (Application Programming Interface) is closed to external traffic on RStudio Server.
  # No problem though! The "out-of-band" authentication seamlessly just redirects any authentication requests to your local computer (and it will just pop up another tab in your internet browser)
```
...
First 5 entries in the uncleaned `Text` column: `r print(envDF$Text[1:5])`

First 5 entries in the cleaned `Text` column: `r print(envDF_tidy$Text[1:5])`
